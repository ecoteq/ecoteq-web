...
const Flame = () => <Ico><path d="M8.5 14.5A2.5 2.5 0 0 0 11 12c0-1.38-.5-2-1-3-1.072-2.143-.224-4.054 2-6 .5 2.5 2 4.9 4 6.5 2 1.6 3 3.5 3 5.5a7 7 0 1 1-14 0c0-1.153.433-2.294 1-3a2.5 2.5 0 0 0 2.5 2.5Z"/></Ico>;
...
const PRESSES = [
  { cat: "Gyűrűsmatricás · ipari", title: "PM-Ring 90", desc: "Gyűrűsmatricás pelletáló nagy volumenű faipari és biomassza pelletgyártáshoz — folyamatos, kiszámítható kimenet.", group: "Ipari", img: PHOTO + "pelletizing-line.png", rate: "1 200 kg/h" },
  { cat: "Gyűrűsmatricás · kompakt", title: "PM-Ring 45", desc: "Kompakt gyűrűsmatricás sor közepes üzemméretre, hűtővel és csomagolással.", group: "Ipari", img: PHOTO + "pelletizing-line.png", rate: "600 kg/h" },
  { cat: "Síkmatricás · kisüzemi", title: "FlatPress 30", desc: "Síkmatricás pelletáló kisüzemi és gazdasági pelletgyártáshoz, gyors megtérüléssel.", group: "Kisüzemi", img: PHOTO + "pelletizing-line.png", rate: "300 kg/h" },
  { cat: "Pellet-hűtő & rosta", title: "CoolSift 2.0", desc: "Ellenáramú pellet-hűtő és rosta a kemény, pormentes, kész pellethez.", group: "Kiegészítő", img: PHOTO + "pelletizing-line.png", rate: "—" },
];
  ...
  return (
    <aside style={{ position: "fixed", left: 0, top: 0, bottom: 0, width: "var(--sidebar-w)", background: "var(--color-surface-dark)",
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14, alignItems: "center" }}>
        <a href="#" aria-label="info@pelletgyarto.hu" style={railIconStyle} className="rail-ic"><span style={{width:18,height:18,display:"inline-flex"}}><Mail/></span></a>
    ...
    <header style={{ position: "sticky", top: 0, zIndex: 30, background: "var(--color-header-bg, rgba(255,255,255,0.92))", backdropFilter: "blur(8px)",
      ...
      <span style={{ display: "inline-flex", alignItems: "center", gap: 9 }}>
        ...
        <span style={{ fontFamily: "var(--font-display)", fontWeight: 800, fontSize: 21, letterSpacing: "-0.01em", color: "var(--color-text)" }}>
          pelletgyarto<span style={{ color: "var(--color-accent)" }}>.hu</span>
        </span>
      </span>
...
function Hero({ onQuote }) {
  return (
    <section style={{ position: "relative", minHeight: 560, display: "flex", alignItems: "flex-end", overflow: "hidden" }}>
      <div style={{ position: "absolute", inset: 0, background: `url(${PHOTO}pelletizing-line.png) center/cover` }} />
      ...
      <div style={{ position: "relative", padding: "0 64px 72px", maxWidth: 840 }}>
        <Tag>Pelletálás · biomassza & fa</Tag>
        <h1 style={{ color: "#fff", fontSize: 72, lineHeight: 0.98, letterSpacing: "-0.02em", margin: "20px 0 0", fontWeight: 800 }}>
          A biomasszából <span style={{ color: "var(--color-accent)" }}>prémium pellet</span>
        </h1>
        <p style={{ color: "rgba(255,255,255,0.85)", fontSize: 19, lineHeight: 1.55, maxWidth: 640, marginTop: 18 }}>
          Sík- és gyűrűsmatricás pelletálók faipari forgácsból, fűrészporból és mezőgazdasági melléktermékből — kisüzemitől az ipari sorig, hűtéssel és csomagolással, magyar mérnöki támogatással.
        </p>
        <div style={{ display: "flex", gap: 14, marginTop: 30, flexWrap: "wrap" }}>
          <Button variant="primary" size="lg" iconRight={<ArrowRight/>} onClick={onQuote}>Ajánlatot kérek</Button>
          <Button variant="outline-light" size="lg" iconLeft={<Download/>}>Pelletáló-katalógus</Button>
        </div>
      </div>
    </section>
  );
...
function Stats() {
  return (
    <section style={{ background: "var(--color-surface-dark)", padding: "40px 64px", display: "flex", gap: 64, flexWrap: "wrap" }}>
      <StatBlock tone="dark" value="1 200" suffix="kg/h" label="csúcskapacitás pelletálónként" />
      <StatBlock tone="dark" value="2" label="matrica-típus üzemméret szerint" />
      <StatBlock tone="dark" value="6" suffix="mm" label="szabványos pellet-átmérő" />
      <StatBlock tone="dark" value="18" suffix="hó" label="tipikus megtérülés" />
    </section>
  );
}

/* ---------- Press grid ---------- */
function Presses({ onQuote }) {
  ...
  return (
    <section style={{ padding: "72px 64px", maxWidth: 1280, margin: "0 auto" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", gap: 24, flexWrap: "wrap", marginBottom: 32 }}>
        <SectionHeading overline="Pelletálók" title="Pelletáló minden üzemmérethez"
          lead="A síkmatricás kisüzemi géptől az ipari gyűrűsmatricás sorig — kategória szerint." />
        <div style={{ minWidth: 220 }}>
          <Select label="Üzemméret" value={filter} onChange={(e) => setFilter(e.target.value)} options={groups} />
        </div>
      </div>
...
function Process() {
  const items = [
    { t: "Kemény, pormentes pellet", d: "A kondicionálás és a helyes matrica stabil, tömör pelletet ad — magas fűtőérték, kevés töredék." , ic: <Flame/> },
    { t: "Mérnöki kiválasztás", d: "Anyag, nedvesség és volumen alapján méretezzük a pelletálót és a matricát — hogy valóban visszahozza az árát." , ic: <Gauge/> },
    { t: "Körforgásos haszon", d: "A saját melléktermék lesz a tüzelőanyag vagy eladható termék — kevesebb hulladék, új bevétel." , ic: <Recycle/> },
  ];
  return (
    <section style={{ background: "var(--color-bg-subtle)", padding: "72px 64px" }}>
      <div style={{ maxWidth: 1280, margin: "0 auto" }}>
        <SectionHeading align="center" style={{ margin: "0 auto 40px" }} overline="Miért pellet?"
          title="A melléktermékből érték" lead="Nem csak pelletálót szállítunk — végigvisszük a projektet az anyagvizsgálattól az üzemeltetésig." />
  ...
  return (
    <section style={{ background: "var(--color-surface-dark)", padding: "72px 64px", position: "relative", overflow: "hidden" }}>
      <div style={{ maxWidth: 1280, margin: "0 auto", display: "grid", gridTemplateColumns: "1.1fr 1fr", gap: 56, alignItems: "center" }}>
        <div>
          <SectionHeading tone="dark" overline="Kapcsolat" title="Beszéljük át a pellet-projektjét"
            lead="Küldje el az alapanyagot és a kívánt volument — 24 órán belül pelletáló-javaslattal és ár-becsléssel jelentkezünk." />
          <div style={{ display: "flex", gap: 14, marginTop: 28, flexWrap: "wrap" }}>
            <ContactPill icon={<Mail/>} href="#">info@pelletgyarto.hu</ContactPill>
        ...
        <Card padding={28}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            ...
            <Select label="Alapanyag" options={["Faipari forgács","Fűrészpor","Szalma / mezőgazdasági","Papír / karton","Fém forgács"]} />
          </div>
          <div style={{ marginTop: 16 }}><Input label="Becsült volumen" placeholder="pl. 500 kg/nap" /></div>
    ...
    <footer style={{ background: "var(--color-feature-deep, var(--navy-900))", color: "rgba(255,255,255,0.7)", padding: "48px 64px" }}>
      <div style={{ maxWidth: 1280, margin: "0 auto", display: "flex", justifyContent: "space-between", gap: 40, flexWrap: "wrap", alignItems: "flex-start" }}>
        <div style={{ maxWidth: 320 }}>
          ...
          <p style={{ marginTop: 14, fontSize: 14, lineHeight: 1.6 }}>Az ECOTEQ pelletálási üzletága. Sík- és gyűrűsmatricás pelletálók magyar mérnöki támogatással.</p>
        </div>
        <div style={{ display: "flex", gap: 56, flexWrap: "wrap" }}>
          <div>
            ...
            {subs.map((s) => <div key={s} style={{ fontSize: 14, padding: "4px 0", color: s === "pelletgyarto.hu" ? "var(--color-accent)" : "inherit", fontWeight: s === "pelletgyarto.hu" ? 700 : 400 }}>{s}</div>)}
          </div>
          <div>
            <div style={{ color: "#fff", fontWeight: 700, fontSize: 13, letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 12 }}>Kapcsolat</div>
            <div style={{ fontSize: 14, padding: "4px 0" }}>info@pelletgyarto.hu</div>
            <div style={{ fontSize: 14, padding: "4px 0" }}>+36 00 000 000</div>
          </div>
        </div>
      </div>
      <div style={{ maxWidth: 1280, margin: "28px auto 0", paddingTop: 20, borderTop: "1px solid rgba(255,255,255,0.1)", fontSize: 13 }}>© 2026 ECOTEQ Kft. — pelletgyarto.hu — Minden jog fenntartva.</div>
    </footer>
  );
}
...
function QuoteModal({ open, onClose }) {
  ...
  if (!open) return null;
  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(0,20,46,0.6)", backdropFilter: "blur(3px)", zIndex: 60,
        ...
        <div style={{ padding: 24 }}>
          {sent ? (
            <div style={{ textAlign: "center", padding: "24px 0" }}>
              ...
              <p style={{ margin: 0, color: "var(--color-text-secondary)" }}>24 órán belül pelletáló-javaslattal jelentkezünk.</p>
            ...
            <>
              ...
              <div style={{ marginTop: 14 }}><Select label="Milyen alapanyaghoz?" options={["Faipari forgács","Fűrészpor","Szalma / mezőgazdasági","Papír / karton","Fém forgács"]} /></div>
...
function PelletgyartoSite() {
  ...
  const [mode, setMode] = React.useState(() => {
    try { return localStorage.getItem("pelletgyarto-mode") || "light"; } catch (e) { return "light"; }
  });
  React.useEffect(() => {
    document.documentElement.dataset.mode = mode;
    try { localStorage.setItem("pelletgyarto-mode", mode); } catch (e) {}
  }, [mode]);
