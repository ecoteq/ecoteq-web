...
const SHREDDERS = [
  { cat: "ISVE · egytengelyes", title: "TR 1100", desc: "Egytengelyes ipari daráló raklaphoz, fahulladékhoz és vegyes anyaghoz — erős, tömör kivitel.", group: "Egytengelyes", img: PHOTO + "shredder.png", rate: "1 200 kg/h" },
  { cat: "ISVE · egytengelyes", title: "TR 1500 HD", desc: "Nagy átmérőjű rotor és erősített rosta nehéz, folyamatos ipari terheléshez.", group: "Egytengelyes", img: PHOTO + "shredder.png", rate: "2 000 kg/h" },
  { cat: "ISVE · négytengelyes", title: "QR 800", desc: "Négytengelyes aprító kontrollált, egyenletes szemcsemérethez — utófeldolgozásra kész.", group: "Négytengelyes", img: PHOTO + "shredder.png", rate: "900 kg/h" },
  { cat: "AL-KO · forgácsoló", title: "Power Chip 75", desc: "Kompakt forgácsoló asztalos- és faipari hulladékhoz, halk üzem, alacsony karbantartás.", group: "Forgácsoló", img: PHOTO + "shredder.png", rate: "400 kg/h" },
];
  ...
  return (
    <aside style={{ position: "fixed", left: 0, top: 0, bottom: 0, width: "var(--sidebar-w)", background: "var(--color-surface-dark)",
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14, alignItems: "center" }}>
        <a href="#" aria-label="info@iparidaralo.hu" style={railIconStyle} className="rail-ic"><span style={{width:18,height:18,display:"inline-flex"}}><Mail/></span></a>
    ...
    <header style={{ position: "sticky", top: 0, zIndex: 30, background: "var(--color-header-bg, rgba(255,255,255,0.92))", backdropFilter: "blur(8px)",
      ...
      <span style={{ display: "inline-flex", alignItems: "center", gap: 9 }}>
        ...
        <span style={{ fontFamily: "var(--font-display)", fontWeight: 800, fontSize: 21, letterSpacing: "-0.01em", color: "var(--color-text)" }}>
          iparidaralo<span style={{ color: "var(--color-accent)" }}>.hu</span>
        </span>
      </span>
  ...
  return (
    <section style={{ position: "relative", minHeight: 560, display: "flex", alignItems: "flex-end", overflow: "hidden" }}>
      ...
      <div style={{ position: "relative", padding: "0 64px 72px", maxWidth: 840 }}>
        <Tag>Ipari darálás · ISVE · AL-KO</Tag>
        <h1 style={{ color: "#fff", fontSize: 72, lineHeight: 0.98, letterSpacing: "-0.02em", margin: "20px 0 0", fontWeight: 800 }}>
          Az aprítás <span style={{ color: "var(--color-accent)" }}>az első lépés</span>
        </h1>
        <p style={{ color: "rgba(255,255,255,0.85)", fontSize: 19, lineHeight: 1.55, maxWidth: 640, marginTop: 18 }}>
          Egy- és négytengelyes ipari darálók fához, raklaphoz, vegyes és nehéz hulladékhoz — olasz ISVE robusztusság, magyar mérnöki támogatással a tervezéstől a karbantartásig.
        </p>
        <div style={{ display: "flex", gap: 14, marginTop: 30, flexWrap: "wrap" }}>
          <Button variant="primary" size="lg" iconRight={<ArrowRight/>} onClick={onQuote}>Ajánlatot kérek</Button>
          <Button variant="outline-light" size="lg" iconLeft={<Download/>}>Daráló-katalógus</Button>
        </div>
      </div>
    </section>
  );
...
function Stats() {
  return (
    <section style={{ background: "var(--color-surface-dark)", padding: "40px 64px", display: "flex", gap: 64, flexWrap: "wrap" }}>
      <StatBlock tone="dark" value="2 000" suffix="kg/h" label="csúcskapacitás darálónként" />
      <StatBlock tone="dark" value="4" label="rotor-konfiguráció üzemméret szerint" />
      <StatBlock tone="dark" value="15" suffix="mm" label="kontrollált szemcseméret" />
  ...
  return (
    <section style={{ padding: "72px 64px", maxWidth: 1280, margin: "0 auto" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", gap: 24, flexWrap: "wrap", marginBottom: 32 }}>
        <SectionHeading overline="Darálók" title="Daráló minden anyaghoz és volumenhez"
          lead="A kompakt forgácsolótól a nagy teljesítményű ipari aprítóig — rotor-típus szerint." />
        <div style={{ minWidth: 220 }}>
          <Select label="Rotor-típus" value={filter} onChange={(e) => setFilter(e.target.value)} options={groups} />
        </div>
      </div>
...
function Process() {
  const items = [
    { t: "Egyenletes szemcse", d: "A kontrollált aprítás stabil szemcseméretet ad — ez a feltétele a jó brikettnek, pelletnek vagy komposztnak." , ic: <Scissors/> },
    { t: "Anyagra méretezve", d: "Rotor, kés és rosta az alapanyaghoz: fa, raklap, vegyes vagy nehéz hulladék — valós áteresztéssel." , ic: <Gauge/> },
    { t: "Az értéklánc kezdete", d: "A darálás az első lépés: a feldolgozott anyag tüzelőanyag, alapanyag vagy eladható termék lesz." , ic: <Recycle/> },
  ];
  return (
    <section style={{ background: "var(--color-bg-subtle)", padding: "72px 64px" }}>
      <div style={{ maxWidth: 1280, margin: "0 auto" }}>
        <SectionHeading align="center" style={{ margin: "0 auto 40px" }} overline="Miért aprítás?"
          title="A feldolgozás első lépése" lead="Nem csak darálót szállítunk — méretezzük, beüzemeljük és életcikluson át támogatjuk." />
      ...
      <div style={{ maxWidth: 1280, margin: "0 auto", display: "grid", gridTemplateColumns: "1.1fr 1fr", gap: 56, alignItems: "center" }}>
        <div>
          ...
          <div style={{ display: "flex", gap: 14, marginTop: 28, flexWrap: "wrap" }}>
            <ContactPill icon={<Mail/>} href="#">info@iparidaralo.hu</ContactPill>
        ...
        <Card padding={28}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            ...
            <Select label="Alapanyag" options={["Fa / raklap","Vegyes hulladék","Műanyag","Papír / karton","Nehéz / fém"]} />
          </div>
          <div style={{ marginTop: 16 }}><Input label="Becsült volumen" placeholder="pl. 1 tonna/nap" /></div>
    ...
    <footer style={{ background: "var(--color-feature-deep, var(--navy-900))", color: "rgba(255,255,255,0.7)", padding: "48px 64px" }}>
      <div style={{ maxWidth: 1280, margin: "0 auto", display: "flex", justifyContent: "space-between", gap: 40, flexWrap: "wrap", alignItems: "flex-start" }}>
        <div style={{ maxWidth: 320 }}>
          ...
          <p style={{ marginTop: 14, fontSize: 14, lineHeight: 1.6 }}>Az ECOTEQ ipari darálási üzletága. Egy- és négytengelyes aprítók magyar mérnöki támogatással.</p>
        </div>
        <div style={{ display: "flex", gap: 56, flexWrap: "wrap" }}>
          <div>
            ...
            {subs.map((s) => <div key={s} style={{ fontSize: 14, padding: "4px 0", color: s === "iparidaralo.hu" ? "var(--color-accent)" : "inherit", fontWeight: s === "iparidaralo.hu" ? 700 : 400 }}>{s}</div>)}
          </div>
          <div>
            <div style={{ color: "#fff", fontWeight: 700, fontSize: 13, letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 12 }}>Kapcsolat</div>
            <div style={{ fontSize: 14, padding: "4px 0" }}>info@iparidaralo.hu</div>
            <div style={{ fontSize: 14, padding: "4px 0" }}>+36 00 000 000</div>
          </div>
        </div>
      </div>
      <div style={{ maxWidth: 1280, margin: "28px auto 0", paddingTop: 20, borderTop: "1px solid rgba(255,255,255,0.1)", fontSize: 13 }}>© 2026 ECOTEQ Kft. — iparidaralo.hu — Minden jog fenntartva.</div>
    </footer>
  );
}
        ...
        <div style={{ padding: 24 }}>
          {sent ? (
            <div style={{ textAlign: "center", padding: "24px 0" }}>
              ...
              <p style={{ margin: 0, color: "var(--color-text-secondary)" }}>24 órán belül daráló-javaslattal jelentkezünk.</p>
            ...
            <>
              ...
              <div style={{ marginTop: 14 }}><Select label="Milyen anyaghoz?" options={["Fa / raklap","Vegyes hulladék","Műanyag","Papír / karton","Nehéz / fém"]} /></div>
...
function IparidaraloSite() {
  ...
  const [mode, setMode] = React.useState(() => {
    try { return localStorage.getItem("iparidaralo-mode") || "light"; } catch (e) { return "light"; }
  });
  React.useEffect(() => {
    document.documentElement.dataset.mode = mode;
    try { localStorage.setItem("iparidaralo-mode", mode); } catch (e) {}
  }, [mode]);
